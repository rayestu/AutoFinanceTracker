 ...
 * Transactions occurring on or after this date will be processed.
 */
...
function runGmailProcessor() {
  ...
  // Convert START_DATE to Gmail query format (YYYY/MM/DD)
  const gmailDate = START_DATE.replace(/-/g, '/');
  const query = `label:transaction-notices after:${gmailDate}`;
  
  const threads = GmailApp.search(query, 0, 50);
  console.log(`🔍 Gmail: Searching with query "${query}". Found ${threads.length} threads.`);
...
function runWhatsAppZipProcessor() {
  ...
  const thresholdDate = new Date(START_DATE);

  // Filter for transactions starting from the variable START_DATE
  const lines = chatText.split('\n').filter(line => {
    ...
    if (dateMatch) {
      ...
      return lineDate >= thresholdDate;
    }
    return false;
  });

  console.log(`🔍 WhatsApp: Processing ${lines.length} lines from ${START_DATE} onwards.`);
